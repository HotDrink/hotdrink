...
shows how to create variables and constraints, and how you can directly
...
and /result/.  Each variable holds a string.  The value of /emails/ is a
comma-delimited list of email addresses.  The value of /filter/ is any string.
The value of /result/ is the comma-delimited list of just those addresses in
/emails/ which contain the string in /filter/.

Usually in a HotDrink application we bind variables to the View so that
...
The entire source code for this example is shown below.  (Click the "show"
links to view the code.)  To clarify, we have broken code into three sections:
the HTML that defines the View, the JavaScript that defines the View-Model,
and the JavaScript that binds the View to the View-Model.
...
<table>
  ...
  <tr>
    <th>Emails:</th>
    <td><span id="emailsSpan1"></span></td>
    <td><input type="button" value="Set" onclick="setEmails1()"/></td>
  </tr>
  <tr>
    <th>Filter:</th>
...
// Define the model
var context = new hd.ContextBuilder() (ref:builder)
    .variable( 'emails', 'joe@foo.com, sue@fum.edu, eve@foo.com, bob@baz.org' ) (ref:varemails)
    .variable( 'filter', 'foo.com' ) (ref:varfilter)
    .variable( 'result' ) (ref:varresult)

    .constraint( 'emails, filter, result' ) (ref:con)
    .method( 'emails, filter -> result', function( emails, filter ) { (ref:mth1)
...
pm.update(); (ref:update1)
#+END_SRC

#+CAPTION: /Binding/
#+BEGIN_SRC js -n -r :exports code :tangle tangle/whatever.js :padline no
window.addEventListener( 'load', function() {
  setSpan( 'emailsSpan1', context.emails.get() );
  setSpan( 'filterSpan1', context.filter.get() );
} )
...
Recall from [[file:intro.org][the introduction]] that there are two steps to using HotDrink.  The
first is to create variables and constraints as elements of a /context/.  The
second is to create a /property model/ object and add the context to it.  The
property model is responsible for enforcing the constraints of the context.

We create a context using a factory object called ~ContextBuilder~.  This
object has member functions for creating the variables and constraints that
...
retrieve the completed context.  The code below illustrates how this might be
done.
...
simply begin invoking member functions on it.  [[(varlist)][Line (varemails)]]
...
(varemails)]] of the view-model creates a variable named ~emails~ which is
