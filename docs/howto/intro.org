...
The task of building a context is rather complex.  To help, HotDrink provides
...
<table>
  ...
  <tr>
    <th>Check-Out:</th>
    <td><input type="text" data-bind="hd.date( checkout )"/></td>
  </tr>
  <tr>
    <th>Nights:</th>
    <td><<nights>></td>
  ...
  <tr>
    <td colspan="2" align="right">
      <input type="button" value="Submit" data-bind="hd.click( reportPrice )"/>
    </td>
  </tr>
</table>
#+END_SRC
...
#+NAME: checkin
#+BEGIN_SRC html
<input type="text" data-bind="hd.date( checkin )"/>
#+END_SRC
...
~hd.num~ function indicates that the number is to have zero digits after the
decimal point---i.e., it is to be an integer.

#+NAME: nights
#+BEGIN_SRC html
<input type="text" data-bind="hd.num( nights, 0 )"/>
#+END_SRC
...
#+NAME: rate
#+BEGIN_SRC html
<select data-bind="hd.value( type )">
#+END_SRC

The view contains one other element bound to a variable, though it is not an
input element.  The following ~<span>~ tag contains a binding specification
indicating the ~price~ variable is to be the contents of the tag, and that it
is to be interpreted simply as text---i.e., no HTML tags, etc.  The call to
~hd.fix( 2 )~ indicates that the variable is to be converted from a number to
a string containing two digits after the decimal point.

#+NAME: price
#+BEGIN_SRC html
<span data-bind="hd.text( price, hd.fix( 2 ) )"></span>
#+END_SRC
...
define our property model.  We create these as members of a context using a
context builder.  The general strategy for constructing a context is: (1.)
create a new builder object, (2.) use the builder member functions to create
variables, constraints, etc., and (3.) call the ~context~ builder member
...
Once you have a context, you can create a property model and add the variables
and constraints of the context to the property model, like so:
...
The ~performDeclaredBindings~ function causes HotDrink to search a part of the
...
specification.  The first parameter is a context used to look up any variable
...
#  LocalWords:  checkin oneDayMs setHours ContextBuilder propmod num getTime
