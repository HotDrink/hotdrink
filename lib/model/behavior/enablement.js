(function () {
  ...
  var enablementBehavior = hd.enablement = {};

  hd.behaviors.push(enablementBehavior);

  var deepTransformVariables = function deepTransformVariables(model, f) {
    if (!model) {
      return;
    }
    ...
    if (Array.isArray(model)) {
      model.forEach(function (submodel) {
        deepTransformVariables(submodel, f);
      });
    } else if (typeof model === "object") {
      Object.keys(model).forEach(function (v) {
        deepTransformVariables(model[v], f);
      });
    } else if (hd.isProxy(model)) {
      f(model.unwrap());
    }
  };

  enablementBehavior.exitCtor = function exitCtor(model) {
    deepTransformVariables(model, function (vv) {
      if (vv.cellType === "interface") {
        vv.canBeDisabled = true;
      }
    });
  };

  enablementBehavior.update
    = function update(touchedSet, executedSet, changedSet, timestamp)
  {
    ...
    executedSet.forEach(function (mm) {
      /* Only need to check the mutual difference (union minus intersection),
       * but not sure calculating that worth the effort. */
    ...
    delete this.changedSet;
    LOG("Finished enablement analysis.");
  };

  enablementBehavior.maybeMarkCanBeDisabled
    = function maybeMarkCanBeDisabled(vv)
  {
    ...
    /* Only variables chosen by the programmer can be disabled. */
    if (vv.cellType !== "interface" ||
      !vv.hasOwnProperty("canBeDisabled"))
    {
      return;
    }
  ...
  enablementBehavior.canBeRelevant = function canBeRelevant(vv) {
   ...
   */
  enablementBehavior.isRelevant = function isRelevant(vv) {
    ...
    /* Base case: commands and unsatisfied preconditions are relevant. */
    if (vv.cellType === "command" || vv.cellType === "precondition") {
      LOG(vv + " is relevant");
