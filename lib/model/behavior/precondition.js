/**
 * @fileOverview <p>{@link hotdrink.model.behavior.Precondition}</p>
 * @author John Freeman
 */

//provides("hotdrink.model.behavior.Precondition");
...
(function () {
  ...
  Precondition.prototype.extend = function extend(factory) {

    factory.precondition = function precondition(commands, fn) {
      ...
      commands = commands.map(function (proxy) {
        ASSERT(this.isCommand(proxy),
          "expected precondition to guard command");
        return proxy.getMore();
      }, this);

      var vv = this.addComputedVariable("precondition", fn);
      vv.guarded = commands;
    };
  ...
  namespace.open("hotdrink.model.behavior").Precondition = Precondition;
