(function () {
  ...
  Solver.prototype.addVariable = function addVariable(cc, vv) {
    ...
    if (!vvv) {
      ...
      DEBUG_BEGIN;
      var prioritySorted = this.priority.slice();
      DEBUG_END;
      this.priority.sort(definedLater);
      ASSERT(prioritySorted.equals(this.priority),
        "expected idempotent sort");
      this.priority.sort(definedLater);
      ASSERT(prioritySorted.equals(this.priority),
        "expected idempotent sort");
  ...
  var definedLater = function definedLater(vvva, vvvb) {
    ASSERT(typeof vvva.outer.orderNo === "number" && vvva.outer.orderNo >= 0,
      "expected " + vvva + " to have an orderNo");
    ASSERT(typeof vvvb.outer.orderNo === "number" && vvvb.outer.orderNo >= 0,
      "expected " + vvvb + " to have an orderNo");
    ASSERT(vvva.outer.inner === vvva,
      "expected mutual reference for" + vvva);
    ASSERT(vvvb.outer.inner === vvvb,
      "expected mutual reference for" + vvvb);
