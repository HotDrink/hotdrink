(function () {
  ...
  var removeAll = function removeAll(items) {
    ...
    var removes = [];
    var i       = list.length - 1;
    ASSERT(Array.isArray(items), "removeAll argument must be an array");

    for (; i >= 0; --i) {
      if(items.indexOf(list[i]) !== -1) {
        list.splice(i, 1);
        removes.push({ index: i, howMany: 1 });
      }
    }
  ...
  var sort = function sort(f) {
    ...
    list.sort(f);
    vv.changeEvent = {
      removes: [{ index: 0, howMany: list.length }],
      adds:  [{ index: 0, howMany: list.length }]
    };
    runtime.touch(vv);
  };
