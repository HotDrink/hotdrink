(function () {
  ...
  proxies.makeInterfaceProxy = function makeInterfaceProxy(vv) {
  ...
  proxies.makeLogicProxy = function makeLogicProxy(vv, set) {
    ASSERT(vv.cellType === "logic",
      "cannot make variable proxy for " + vv);
  ...
  hd.isVariable = function isVariable(proxy) {
    return hd.isProxy(proxy) && (proxy.unwrap().cellType !== "command");
  };

  proxies.makeCommandProxy = function makeCommandProxy(vv) {
    ASSERT(vv.cellType === "command", "cannot make command proxy for " + vv);
