...
(function () {
  ...
  var get = function get(vv) {
    ...
    if (caller) {
      ...
      if (!contains(caller.inputsUsed, vv)) caller.inputsUsed.push(vv);
      if (!contains(vv.usedBy, caller)) vv.usedBy.push(caller);
    }

    DEBUG_BEGIN;
    if (vv.valuePrev !== vv.value) {
  ...
  var set = function set(vv) {
    ...
    LOG("set " + vv + " : " + 
        ((typeof vv.valuePrev === "function")
         ? "<function>" : JSON.stringify(vv.valuePrev)) + " ==> " +
        ((typeof vv.value === "function")
         ? "<function>" : JSON.stringify(vv.value)));

    /* Assign. */
    vv.valuePrev = vv.value;
      ...
      if (mm.constraint) {
        ...
        if (mmSelectedPrev) {
          ...
          mmSelectedPrev.inputsUsed.forEach(function (vv) {
            remove(vv.usedBy, mmSelectedPrev)
          });
        }
      }
    });
  ...
  var isInUpdate = function isInUpdate() { return inUpdateMark; };

  /**
   * @param {hotdrink.graph.Model} model
   * @class
   *   An Evaluator is an interface to a global singleton. It carries no
   *   instance state. It will operate on any constraint graph.
   */
  var Evaluator = function Evaluator() {
  };

  /* Class methods that we will pretend are instance methods. */
  Evaluator.prototype.update = update;
  Evaluator.prototype.get = get;
  Evaluator.prototype.isInUpdate = isInUpdate;
  Evaluator.prototype.getTimestamp
