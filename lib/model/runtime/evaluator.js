(function () {
  ...
  var maybeExecute = function maybeExecute(mm) {
    LOG("maybeExecute: " + mm.toSignature());
    ...
    LOG("maybeExecute: checking that inputs of " + mm + " are ready...");
    var ready = mm.inputsUsed.every(function (uu) {
      maybeSet(uu);
      return uu.lastUpdated > timestamp;
    });
    if (!ready) {
      LOG("maybeExecute: " + mm + " has inputs updating");
      return;
    }
    LOG("maybeExecute: inputs of " + mm + " are ready");
