(function () {
   ...
   * have both a "gray" (timestamp) and "black" (timestamp + 1) color for the
   * marks.
   */
  var timestamp = 0;
   ...
   * question has been updated, then they will return only after executing
   * the update. */
  ...
  var maybeSet = function maybeSet(vv) {
    ...
    if (vv.lastUpdated >= timestamp) {
      LOG("maybeSet: already updated for this evaluation");
      return;
    }
  ...
  var maybeExecute = function maybeExecute(mm) {
    ...
    /* Don't bother unless all of its inputs are ready. */
    /* DANGER: If an input is going to be used, but is not in `inputsUsed`,
     * and it is being updated, we're in trouble. Think of this situation
     * more. Will probably need an execution queue. */
    var ready = mm.inputsUsed.every(function (uu) {
      maybeSet(uu);
      return uu.lastUpdated > timestamp;
    });
    if (!ready) {
      LOG("maybeExecute: " + mm + " has inputs being updated");
      return;
    }
    ...
    if (mm.lastUpdated >= timestamp) {
      LOG("maybeExecute: " + mm +
        (mm.needsExecution ? " is executing" : " already updated"));
      return;
    }
    ...
    if (!mm.needsExecution) {
      ...
      mm.needsExecution = mm.inputsUsed.some(function (uu) {
        /* Do not need to call `maybeSet` here because we did earlier. */
  ...
  var execute = function execute(mm) {
    ...
    ASSERT(mm.lastUpdated === timestamp,
      "execute: do not execute more than once");
    mm.lastUpdated = timestamp + 1;
