var publisher = (function () {
  ...
  /* If called with just an event name, returns the pending draft for that
   * event, creating one if none exists. Otherwise, saves a draft for the
   * event.
   *
   * A draft is an array of arguments that will be passed to each subscriber
   * using Function.prototype.apply. */
  var draft = function draft(evt/*, ...*/) {
    if (arguments.length === 1) {
      return this.drafts[evt] = (this.drafts[evt] || []);
    }
